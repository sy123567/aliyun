...
INCOME_EFFICIENCY_BONUS_THRESHOLD = 5.0
"""yuan/km 超过此值的货源获得效率奖励，鼓励选择高单价货。"""

INCOME_EFFICIENCY_BONUS_PER_UNIT = 15.0
"""每超过阈值 1 yuan/km 的奖励金额（元）。保守设定避免扰乱评分平衡。"""

INCOME_EFFICIENCY_PENALTY_THRESHOLD = 0.0
"""yuan/km 低于此值的货源受到效率惩罚。设为0禁用惩罚（短途低价有时是最优选择）。"""

INCOME_EFFICIENCY_PENALTY_PER_UNIT = 0.0
"""每低于阈值 1 yuan/km 的惩罚金额（元）。设为0禁用。"""

DEADHEAD_HAUL_RATIO_THRESHOLD = 1.2
"""空驶/运距比超过此值时施加额外惩罚。提高至1.2仅惩罚极端情况，避免与pickup_deadhead重复惩罚。"""

DEADHEAD_HAUL_RATIO_PENALTY_COEFF = 0.15
"""空驶比惩罚系数：penalty = cost_per_km × 超额空驶km × 此系数。减半避免过度惩罚。"""

FUTURE_VALUE_HORIZON_MULTIPLIER = 1.1
"""未来位置价值放大系数。保守设定（1.1x）避免过度强调定位而忽略即时收入。"""

FUTURE_VALUE_DENSITY_WEIGHT = 0.15
"""热点货源密度在未来价值估算中的权重。减小避免密度信号噪声干扰。"""

PACE_TARGET_DAILY_INCOME = 6000.0
"""目标日均收入（元）。降低至6000避免节奏检测始终触发（total_gross_income从0开始）。"""

PACE_BEHIND_INCOME_BOOST = 1.05
"""落后收入节奏时，income 权重提升倍率。从1.12降至1.05避免持续扰动。"""

PACE_BEHIND_FUTURE_VALUE_BOOST = 1.1
"""落后收入节奏时，future_value 权重提升倍率。从1.3降至1.1避免过度偏向定位。"""
