...
SOFT_NODRIVE_DEFER_PROXIMITY_MINUTES = 240
"""当前时刻距离软禁行窗口起始 ≤ 此分钟数（且尚未进入窗口）时，软罚乘数随接近程度递增（PR#27: D004 中午延后判断）。

调参（离线确定性复跑，纯正则解析）：90→240 让 D004 提前规避"会跨过中午 12–13 软窗"的订单，
改去接收益更高的单，D004 净收入 +1,864（gross +3,536，软罚仅 +100），其余司机不受影响。
240 与 300 等价（plateau）；≥360 会过度延后反而损失毛收入（-3k），故取 240。"""
...
DAILY_REST_RISK_EARLY_RATIO = 8.0
"""每日休息余量预警触发线：remaining_after < deficit × 该比例 时施加软罚（PR#29: 3.0→4.0）。

调参（离线确定性复跑，纯正则解析）：4→8 让 D002/D006 更早留出连续休息段，
D002 连续歇脚违规 7→1（净收入 +2,996、罚分 1,400→200）、D006 +1,044。
EARLY≥6 且 TIGHT=4 后进入 plateau（6/4 与 8/4、10/5 等价），取 8 留余量。"""

DAILY_REST_RISK_TIGHT_RATIO = 4.0
"""每日休息余量紧迫触发线：remaining_after < deficit × 该比例 时施加强软罚（PR#29: 1.5→2.0）。

调参：2→4 是上面 D006 +1,044 的触发关键（TIGHT=2/3 时 D006 不改善）。
代价：D010 因全局休息调度变化净收入 -2,058（仿真按墙钟隐藏家事偏好，激进休息线在
家事窗口可见前已改变 D010 全月轨迹，局部守卫无法消除该扩散耦合）；但 D010 仍 ¥23,350、
远高于回退 run 的 ¥8,460，且不触发家事 ¥9,000 固定罚（仅缺席分钟罚）。总净收入净增 +1,982。"""
...
PR#32 引入时默认 True，但近 3 次评测记录显示其得不偿失：
- 困境模式仅对有 home_rule 的司机触发（见 model_decision_service），D010 的「家事临时约定」
  会被解析成 home_rule，于是 stranded_home_bonus（+500）在家事窗口把 D010 直接拉回老家，
  抢在「先接配偶再回家」的次序之前抵达 → 触发 spouse_incomplete + 提前离家，单次固定罚 ¥9,000，
  D010 净收入从 ¥21,597 暴跌至 ¥8,460（最近一次评测 20260524_223610 总分 239k 的唯一回退来源）。
- 离线确定性复跑显示该特性对 D003/D005/D007 的「长距脱困」收益并未兑现（completed_orders 为空，
  死区判定不触发），关闭后总净收入反而 +143，无任何司机受损。
故默认关闭；代码保留以便 income 回传修复后重新评估。"""
