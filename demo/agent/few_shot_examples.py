"""Few-shot 示例学习模块：从 Run3/Run4/Run5/Run6/Run7 仿真日志中提取的真实决策案例。
...
- Run7 (Few-shot v2): 净收入 242,047 (-0.3%)，罚分 23,650
- Master 基线: 净收入 242,776，罚分 21,850

Run7 修复（v1→v2 净收入 +3,820 元）：
- LLM跳过阈值 1.5x→2.0x：减少不必要的LLM干预，让评分系统更多自决
- 分数保护阈值 0.8→0.9：LLM不能选分数差距>10%的候选
- system prompt强调默认选#1，仅在偏好违规风险时偏离
- D008因此从4000元罚分→3700元，净收入+4049元（最大改善）

核心发现：
1. D009 "23点前到家"规则是最大单点风险（Run5: 28次→Run6: 1次，大幅改善）
2. D004 午休时段(12-13点)规则属于"软偏好"（每天罚100元<接单收益），评分系统故意接受
3. D010 家事约定不在家时间过长，罚5150元（最大单项罚分）
4. LLM干预越少越好——评分系统经10轮校准已高度优化
