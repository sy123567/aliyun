...
def resolve_adaptive_weights(
    ...
    if remaining_days <= config.MONTH_END_REMAINING_DAYS:
        weights = weights.scaled(horizon_risk=config.ADAPTIVE_WEIGHT_MONTH_END_HORIZON_SCALE, future_value=config.ADAPTIVE_WEIGHT_MONTH_END_FUTURE_SCALE)
    ...
    if _is_preference_near_violation(rules, memory):
        ...
        if sim_day >= config.MONTHLY_DAY_OFF_LATE_MONTH_DAY:
            weights = weights.scaled(preference_risk=config.ADAPTIVE_WEIGHT_PREF_RISK_LATE_MONTH_SCALE)
        else:
            weights = weights.scaled(preference_risk=config.ADAPTIVE_WEIGHT_PREF_RISK_NORMAL_SCALE)

    # 货源稀缺
    if visible_cargo_count < config.SCARCE_CARGO_THRESHOLD:
        weights = weights.scaled(pickup_deadhead=config.ADAPTIVE_WEIGHT_SCARCE_PICKUP_SCALE, reposition_gain=config.ADAPTIVE_WEIGHT_SCARCE_REPOSITION_SCALE)
...
def score_take_order(
    ...
    if rules.home_rule is not None:
        ...
        if can_reach_home_by_deadline and not end_in_home:
            ...
            if 0 < slack <= config.HOME_RULE_TIGHT_SLACK_MINUTES:
                severity = max(0.3, 1.0 - slack / config.HOME_RULE_TIGHT_SLACK_MINUTES)
    ...
    horizon_minutes_ahead = min(config.FUTURE_VALUE_HORIZON_MAX_MINUTES, max(config.FUTURE_VALUE_HORIZON_MIN_MINUTES, occupied_minutes // 2))
    future_value = (
        memory.hotspot_value(end_lat, end_lng) + config.FUTURE_VALUE_HOUR_SIGNAL_WEIGHT * hour_signal
    ...
    if rules.home_rule is not None:
        ...
        # 施加渐进惩罚（而非等到已过 home_by_hour 才反应）。
            ...
            if slack_after_repo < 0:
                ...
                severity = max(0.3, 1.0 - slack_after_repo / config.HOME_RULE_REPO_TIGHT_SLACK_MINUTES)
                breakdown["home_rule_tight_slack_penalty"] = -float(rules.home_rule.penalty_amount or 600.0) * severity
