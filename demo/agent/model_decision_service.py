...
1. 静态解析层（兜底）：大模型把自然语言偏好解析成结构化 DriverRules
   （休息窗 / 禁驶窗 / 禁接货类 / 禁接区域 / 品类指标等），失败时退回正则。
   解析结果只作为**保守下限**，不再要求覆盖所有偏好类型。
2. 每日合规计划层（核心，新）：每个仿真日开始时，把**偏好原文** + 当天日历
   （几月几号、星期几）+ 月度合规进度交给 LLM，产出"今天"的具体约束
   （今日禁驶时段、今日必须做/避免的事、品类缺口提醒）。任意措辞、任意
   条件式偏好（周末不同时段、特定日期事件、月度结转）都在这一层被按天
   具象化，不依赖固定 schema 能否表达。
3. 确定性执行层：调度器与各动作守卫使用"静态窗 ∪ 当日计划窗"的**有效
   禁驶窗**硬性拦截违规动作；候选货源按净收益择单。计划层失效时自动
   退化为纯静态规则（fail-safe，宁可保守也不放飞）。
"""
