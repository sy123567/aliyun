...
def test_rank_score_chain_depth_default_off_is_noop() -> None:
    ...
    count must not change the score (strict no-op). Reverted from the gross
    push v5 on-by-default after the platform A/B regressed (notes §-14): this
    lever is NOT penalty-neutral in practice, so it ships off."""
    assert mds._CHAIN_DEPTH_WEIGHT == 0.0, "default AGENT_CHAIN_DEPTH_WEIGHT must be 0 (off; see notes §-14)"
...
def test_nearby_chain_liquidity_default_off_is_noop() -> None:
    ...
    hub still earns no near-credit (strict no-op). Reverted from the gross push
    v5 on-by-default after the platform A/B regressed (notes §-14)."""
    assert mds._CHAIN_NEAR_WEIGHT == 0.0, "default AGENT_CHAIN_NEAR_WEIGHT must be 0 (off; see notes §-14)"
